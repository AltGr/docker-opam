...
* install Docker (`curl -s http://get.docker.io/ubuntu/ | sudo sh` on Ubuntu or see http://docs.docker.io/en/latest/installation/)
...
* run your container: `docker run -t -i opamdock/browse`
`-t` and `-i` are needed to bind a terminal to the docker instance.
